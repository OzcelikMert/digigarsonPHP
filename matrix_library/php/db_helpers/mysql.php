...
abstract class mysql extends helper {
    ...
    function db_insert(
        ...
        try{
            ...
            foreach ($data as $single_key => $single_value) {
                if(is_array($single_value)){
                    ...
                    foreach ($single_value as $multiple_key => $multiple_value){
                        ...
                        $sql_values .= self::convert_varchar($multiple_value).",";
                    }
                    $sql_values = substr($sql_values, 0, -1);
                    $sql_values .= "),";
                }else{
                    $sql_insert .= "$single_key,";
                    $sql_values .= self::convert_varchar($single_value).",";
                }
            }
            ...
            if(strlen($sql_insert) > 0 && strlen($sql_values) > 0){
                ...
                $this->check_show_sql($results->sql, $sql, $just_show_sql);
                if(!$just_show_sql) {
    ...
    function db_select(
        ...
        try{
            ...
            $sql = "select {$columns_} from {$table_name} {$joins} {$this->check_condition(self::WHERE, $where)} {$this->check_condition(self::GROUP_BY, $group_by)} {$this->check_condition(self::ORDER_BY, $order_by)} {$this->check_condition(self::LIMIT, $limit)}".((strlen($union_columns) > 0) ? " union select {$union_columns} from {$union_table_name} {$union_joins} {$this->check_condition(self::WHERE, $union_where)} {$this->check_condition(self::GROUP_BY, $union_group_by)} {$this->check_condition(self::ORDER_BY, $union_order_by)} {$this->check_condition(self::LIMIT, $union_limit)}" : "");
            $this->check_show_sql($results->sql, $sql, $just_show_sql);
            if(!$just_show_sql) {
    ...
    function db_update(
        ...
        try{
            ...
            foreach($columns as $key => $value){
                $columns_ .= "{$key}=".(($checked_varchar) ? self::convert_varchar($value) : $value).",";
            }
            $columns_ = substr($columns_, 0, -1);
            $sql = "update {$table_name} {$joins} set {$columns_} {$this->check_condition(self::WHERE, $where)} {$this->check_condition(self::GROUP_BY, $group_by)}";
            $this->check_show_sql($results->sql, $sql, $just_show_sql);
            if(!$just_show_sql) {
    ...
    function db_delete(
        ...
        $results = new results();
        try{
            $sql = "delete from {$table_name} {$joins} {$this->check_condition(self::WHERE, $where)} {$this->check_condition(self::ORDER_BY, $order_by)} {$this->check_condition(self::LIMIT, $limit)}";
            $this->check_show_sql($results->sql, $sql, $just_show_sql);
            if(!$just_show_sql) {
                if (mysqli_query($this->connect, $sql)) {
                    $results->status = true;
