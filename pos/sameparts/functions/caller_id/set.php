...
use pos\sameparts\functions\caller_id\set\customer;
use pos\sameparts\functions\caller_id\set\cancel;
...
class set_types {
    const CANCEL = 0x0001, CUSTOMER = 0x0002;
}
/* end CONST Values */
...
if(user::check_sent_data([post_keys::SET_TYPE]) && check::check(false)) {
    ...
    switch (user::post(post_keys::SET_TYPE)){
        case set_types::CANCEL:
            (new cancel($db, $sessions, $echo));
            break;
        case set_types::CUSTOMER:
            (new customer($db, $sessions, $echo));
            break;
    }
