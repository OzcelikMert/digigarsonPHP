...
<script>
    ...
    $inv = "invoice";
    if (in_array($_SESSION["branch_id"], [3])){
        $inv = "invoice_new";
        echo '<script>console.log("YENİ YAZDIRMA SİSTEMİ")</script>';
    }
    ?>
    <script src = './assets/scripts/main.js?<?=$v;?>' ></script >
    <script src = './assets/scripts/navbar.js?<?=$v;?>' ></script >
    <!--------------PRINTER --------------->
    <script src = './assets/scripts/printer/<?=$inv?>.js<?=$v?>' ></script >
    ...
    <?php if(unserialize($_SESSION["integrations"]["yemek_sepeti"])->status) {?><script src="./../integrations/companies/integrated/yemek_sepeti/script/initialize.js<?=$v?>"></script><?php } ?> <?php } ?>
