let main = (function () {
  ...
  main.get_catering_related_things = function (
    get_type = main.get_type_for_catering_related_things.ALL,
    async = false
  ) {
    $.ajax({
  ...
  main.get_trust_accounts_related_things = function (
    get_type = main.get_type_for_branch_trust_accounts_related_things.ALL,
    async = false
  ) {
    $.ajax({
  ...
  main.get_notification_related_things = function (
    get_type = main.get_type_for_notification_related_things.ALL,
    async = false
  ) {
    $.ajax({
  ...
  function set_data_list(data, set_type) {
    switch (set_type) {
      ...
      case main.set_type_for_data_list.CATERING_RELATED_THINGS:
        if (variable.isset(() => data.rows.catering_owners.rows))
          main.data_list.CATERING_OWNERS = data.rows.catering_owners.rows;
        if (variable.isset(() => data.rows.catering_questions.rows))
          main.data_list.CATERING_QUESTIONS = data.rows.catering_questions.rows;
        if (variable.isset(() => data.rows.caterings.rows))
          main.data_list.CATERINGS = data.rows.caterings.rows;
        break;
      ...
      case main.set_type_for_data_list.BRANCH_TRUST_ACCOUNTS:
        if (variable.isset(() => data.rows.accounts.rows))
          main.data_list.TRUST_ACCOUNTS = data.rows.accounts.rows;
        if (variable.isset(() => data.rows.payments.rows))
          main.data_list.TRUST_ACCOUNT_PAYMENTS = data.rows.payments.rows;
        break;
      case main.set_type_for_data_list.INTEGRATE_RELATED_THINGS:
        if (variable.isset(() => data.rows.products.rows))
          main.data_list.PRODUCTS_INTEGRATE = data.rows.products.rows;
        if (variable.isset(() => data.rows.options.rows))
          main.data_list.PRODUCT_OPTIONS_INTEGRATE = data.rows.options.rows;
        if (variable.isset(() => data.rows.orders.rows))
          main.data_list.ORDERS_INTEGRATE = data.rows.orders.rows;
        if (variable.isset(() => data.rows.payment_types.rows))
          main.data_list.PAYMENT_TYPES_INTEGRATE = data.rows.payment_types.rows;
        break;
      case main.set_type_for_data_list.NOTIFICATION_TYPES_RELATED_THINGS:
        if (variable.isset(() => data.rows.notification_types.rows))
          main.data_list.NOTIFICATION_TYPES = data.rows.notification_types.rows;
        if (variable.isset(() => data.rows.notifications.rows))
          main.data_list.NOTIFICATIONS = data.rows.notifications.rows;
        break;
    }
  }
  ...
  main.timers = { order: null, notification: null };
  function start_timers() {
    setTimeout(function () {

      if (invoice.auto_print_interval == null) {
        invoice.auto_print();
      }

      if (main.timers.order == null) {
        main.timers.order = setInterval(function () {
          helper.log("order timer");
          main.get_order_related_things();
        }, settings.ajax_timeouts.NORMAL);
      }

      if (main.timers.notification == null) {
        main.timers.notification = setInterval(function () {
          if (app.customize_settings.enableNotifications) {
            helper.log("notification timer");
            main.get_notification_related_things(
              main.get_type_for_notification_related_things.NOTIFICATIONS,
              true
            );
          } else clearInterval(main.timers.notification);
