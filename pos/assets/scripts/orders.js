let orders = (function () {
    ...
    let table_list = {
        ...
        get: function (){
            ...
            function create_element() {
                let element = ``;
                variable_list.NOW_DATE_TIME = new Date(variable.date_format(new Date(), "yyyy-mm-dd HH:MM:00"));
                main.data_list.SECTIONS.forEach(section => {
                    if(section.is_active === 0 || (sections.SELECTED_SECTION > 0 && sections.SELECTED_SECTION !== section.id)) return;
                    ...
                    tables.forEach(table => {
                        ...
                        if(table.id === helper.db.branch_tables.TAKE_AWAY && typeof main.data_list.PERMISSION[10] === "undefined") return;
                        let selected = "";
                        if(self.variable_list.SELECTED_GET_TYPE === self.get_types.TABLE_MOVE && table_detail.variable_list.SELECTED_TABLE_ID === table.id) selected = "selected";
                        ...
                        function get_element(){
                            ...
                            if(last_time !== ""){
                                ...
                                let date_time_diff = variable.diff_minutes(new Date(last_date_time), variable_list.NOW_DATE_TIME);
                                let hour = Math.floor(date_time_diff / 60);
                                let minute = date_time_diff - (hour * 60);
                                time = `${(hour >= 10) ? hour : `0${hour}`}:${(minute >= 10) ? minute : `0${minute}`}`;
                            }
                            element += `
                                <div table-type="${table.type}" order-type="${order_type}" table-id="${table.id}" last-order-id="${last_order_id}" class="${(self.variable_list.SELECTED_GET_TYPE === self.get_types.TABLE || table_detail.variable_list.SELECTED_TABLE_ID === table.id) ? `e_table` : `e_table_move`} order-table order-table-xl ${self.settings.type[table.shape_type]} p-0 ${selected}">
                                    <div class="in ${self.settings.order_type[order_type]} ${bg}">             
                                        <div class="tc-1 w-100 text-center fw-6 ">${table_no}</div>
                                        <div class="tc-2 w-100 size-type-lg fw-6 text-center">${section_type.name}</div>
                                        <div class="tc-3 w-100 size-type-lg fw-6 text-center">
                                           <p class="e_last_time mx-0 mb-0 d-block float-left">${time}</p>
                                           <p class="e_last_price mx-0 mb-0 d-block float-left">${last_total.toFixed(2) + main.data_list.CURRENCY}</p>
                                        </div>
                                        <div class="e_total_price tc-4 w-100 size-type-lg fw-6 text-center">${total.toFixed(2) + main.data_list.CURRENCY}</div>              
                                    </div>
                                </div>
                            `;
                        }
                        ...
                        orders.forEach(order => {
                            ...
                            switch (table.id){
                                ...
                                case helper.db.branch_tables.YEMEK_SEPETI:
                                    table_no = array_list.find(main.data_list.ORDERS_INTEGRATE, order.id, "order_id").order_id_integrate;
                                    break;
                            }
                            last_order_id = order.id;
                            order_type = order.type;
                            let order_products = array_list.find_multi(main.data_list.ORDER_PRODUCTS, order.id, "order_id");
                            order_products.forEach(order_product => {
                                price = (order_product.status === helper.db.order_product_status_types.CATERING) ? 0 : parseFloat(order_product.price);
                            ...
                            if(table.id === helper.db.branch_tables.TAKE_AWAY || table.id === helper.db.branch_tables.YEMEK_SEPETI){
                                get_element();
                                total = 0;
                                payed_total = 0;
                            }
            ...
            function set_events () {
                ...
                $(self.class_list.MODAL_CALLER_CHOOSE_BTN).on("click", function () {
                    ...
                    switch (function_name) {
                        case "search":
                            if(typeof caller_id !== "undefined") caller_id._check($(`${self.id_list.MODAL_CALLER_CHOOSE} input[name='phone']`).val());
                            break;
                        case "new":
                ...
                $(document).on("click",`${self.class_list.TABLE}`,function (){
                    ...
                    if(
                        table_detail.variable_list.SELECTED_TABLE_ID === helper.db.branch_tables.TAKE_AWAY ||
                        table_detail.variable_list.SELECTED_TABLE_ID === helper.db.branch_tables.YEMEK_SEPETI
                    ){ // takeaway
                        table_detail.variable_list.SELECTED_ORDER_ID = parseInt($(this).attr("last-order-id"));
                    }
                    table_detail.variable_list.IS_SELECT_ORDER = false;
                    table_detail.variable_list.SELECTED_TABLE_TYPE = table_detail.variable_list.table_types.DEFAULT;
                    ...
                    switch (type) {
                        ...
                        case helper.db.branch_table_types.OTHER_SALE:
                            let orders = array_list.find_multi(main.data_list.ORDERS, table_detail.variable_list.SELECTED_TABLE_ID, "table_id");
                            table_detail.variable_list.SELECTED_ORDER_ID = (orders.length > 0 ) ? orders[orders.length - 1].id : 0;
                        ...
                        case helper.db.branch_table_types.TAKEAWAY:
                            if(table_detail.variable_list.SELECTED_ORDER_ID > 0) {
                                ...
                                $(table_detail.class_list.TABLE_TITLE_INFO).html(`${caller_id.variable_list.DATA.user[0].name} (${caller_id.variable_list.DATA.user[0].phone})`);
                                table_detail.variable_list.SELECTED_ORDER_ID = 0;
                            }
                        ...
                        case helper.db.branch_table_types.YEMEK_SEPETI:
                            table_detail.variable_list.ORDER_STATUS_TYPES = integrated_companies.yemek_sepeti.orders.variable_list.order_state.DATA;
                            $(`${table_detail.class_list.ORDER_BTN}[table-integrate]`).show();
                            $(`${table_detail.class_list.ORDER_BTN}:not([table-integrate])`).hide();
                            integrations.variable_list.SELECTED_INTEGRATE_TYPE = helper.db.integrate_types.YEMEK_SEPETI;
                ...
                $(document).on("click", self.class_list.TABLE_MOVE, function () {
                    ...
                    let old_table = array_list.find(main.data_list.TABLES, table_detail.variable_list.SELECTED_TABLE_ID, "id");
                    let old_section = array_list.find(main.data_list.SECTION_TYPES, (array_list.find(main.data_list.SECTIONS, old_table.section_id, "id")).section_id, "id");
                    let new_table = array_list.find(main.data_list.TABLES, id, "id");
                    let new_section = array_list.find(main.data_list.SECTION_TYPES, (array_list.find(main.data_list.SECTIONS, new_table.section_id, "id")).section_id, "id");
                    Swal.fire({
                        title: 'Masa Taşıma',
                        text: `'${old_section.name + " " + old_table.no}' isimli masayı '${new_section.name + " " + new_table.no}' isimli masaya taşımak istediğinizden emin misiniz?`,
                        icon: 'warning',
                        ...
                        if (result.value) {
                            set(
                                set_types.TABLE_MOVE,
                                {
                                    ...
                                    "table_move_id": id,
                                    "order_id": (table_detail.variable_list.IS_SELECT_ORDER) ? table_detail.variable_list.SELECTED_ORDER_ID : 0
                                },
                                function (data){
                                    data = JSON.parse(data);
                                    if(data.status){
                                        $(navbar.class_list.BACK_MOVE_TABLE).trigger("click");
                                        main.get_order_related_things(main.get_type_for_order_related_things.ORDER_AND_ORDER_PRODUCTS);
                ...
                $(navbar.class_list.BACK_MOVE_TABLE).on("click", function () {
                    $(`${self.id_list.TABLE_GROUP} ${self.class_list.TABLE}[table-id=${table_detail.variable_list.SELECTED_TABLE_ID}]`).trigger("click");
                });
            }
            self.get();
            set_events();
            setInterval(function (){ self.get() },settings.ajax_timeouts.NORMAL)
        }
    }
    ...
    let table_detail = {
        ...
        get: function (){
            ...
            let table = array_list.find(main.data_list.TABLES, table_detail.variable_list.SELECTED_TABLE_ID, "id");
            let section = array_list.find(main.data_list.SECTION_TYPES, (array_list.find(main.data_list.SECTIONS, table.section_id, "id")).section_id, "id");
            let table_total = 0.0;

            function create_element(){
                let element = ``;

                let orders = array_list.find_multi(main.data_list.ORDERS,
                    (
                ...
                if (
                    ...
                    if(orders.length > 0){
                        switch (self.variable_list.SELECTED_TABLE_ID){
                            ...
                            case helper.db.branch_tables.YEMEK_SEPETI:
                                $(".e_takeaway_confirm_area").hide();
                                let order_integrate  = array_list.find(main.data_list.ORDERS_INTEGRATE, orders[0].id, "order_id");
                        ...
                        $(`${self.class_list.TAKE_AWAY_INFO} [function="address"]`).html(self.variable_list.SELECT_ADDRESS_STRING );
                ...
                orders.forEach(order => {
                    if(order.confirmed_account_id == 0)
                        set(
                            set_types.UPDATE_CONFIRM_ACCOUNT_ID,
                            {"order_id": order.id},
                            function (data) {
                                data = JSON.parse(data);
                    ...
                    if(order_products.length > 0){
                        order_products.forEach(order_product => {
                            ...
                            let order_product_options = array_list.find_multi(main.data_list.ORDER_PRODUCT_OPTIONS, order_product.id, "order_product_id");
                            let price = parseFloat(order_product.price);
                            let catering_class = "";
                            let option_icon = "";
                            if(order_product_options.length > 0 || order_product.comment !== "" || order_product.quantity != 1){
                                option_icon = "<i class='fa fa-bars'></i>";
                            }
                            if(order_product.status === helper.db.order_product_status_types.CATERING){
                                price = 0;
                                catering_class = "bg-c5 pointer-none";
                            }
                            order_product_elements += `
                                ...
                                options='${JSON.stringify(order_product_options)}'>
                                    ...
                                    <td>${(order_product.status === helper.db.order_product_status_types.CATERING) ? "İkram" : price.toFixed(2) + main.data_list.CURRENCY}</td> 
                                </tr>
                            `;
                            total += price;
                        });
                        ...
                        element += `
                            <tbody class="e_order_body" order-id="${order.id}" price="${(total - payed_total)}" style="display: contents;">
                                <tr class="e_order_title ${(self.variable_list.SELECTED_ORDER_ID === order.id) ? `selected` : ``}" style="background: #0d4197">
                                    <td class="e_order_table_hide_column" style="display: none;"></td>
                                    <td colspan="3">
                                        <span>${order_products[0].account_type}: ${order_products[0].account_name} </span> 
                                        <span>No: ${order.no}</span> <span>Toplam: ${((payed_total > 0) ? `<small><del class="mr-2">${total.toFixed(2)}</del></small>` : ``)}<span function="price">${(total - payed_total).toFixed(2)}</span>${main.data_list.CURRENCY}</span>
                                    </td>
                                </tr>
                                ${order_product_elements}
                            </tbody>
            ...
            $(`${self.id_list.TABLE_DETAILS} ${self.class_list.TABLE_TITLE}`).html(`${section.name.toUpperCase()} ${(table.no === 0) ? "" : table.no}`);
            $(`${self.id_list.TABLE_DETAILS} ${self.class_list.TABLE_PRICE_TOTAL} [function='price']`).html(`${table_total.toFixed(2)}`);
            $(`${self.id_list.TABLE_DETAILS} ${self.class_list.TABLE_PRICE_TOTAL} [function='currency']`).html(`${main.data_list.CURRENCY}`);
            $(`
        ...
        get_categories: function(){
            ...
            function create_element(){
                let element =  (self.variable_list.SEARCH_CATEGORY.length === 0)
                    ? ` 
                ...
                main.data_list.PRODUCT_CATEGORIES.forEach(category => {
                    ...
                    if(self.variable_list.SEARCH_CATEGORY.length > 0 && !String(category.name.toLocaleLowerCase("tr")).match(new RegExp(self.variable_list.SEARCH_CATEGORY.toLocaleLowerCase("tr"), "gi"))) return;
        ...
        get_product: function (){
            ...
            function create_element(){
                ...
                if(self.variable_list.SELECTED_CATEGORY > 0){
                    let get_main = array_list.find(main.data_list.PRODUCT_CATEGORIES, self.variable_list.SELECTED_CATEGORY, "id");
                    let get_sub = array_list.find_multi(main.data_list.PRODUCT_CATEGORIES, self.variable_list.SELECTED_CATEGORY, "main_id");
                    ...
                    get_sub.forEach(category => {
                        if(self.variable_list.SEARCH_PRODUCT.length > 0 && !String(category.name.toLocaleLowerCase("tr")).match(new RegExp(self.variable_list.SEARCH_PRODUCT.toLocaleLowerCase("tr"), "gi"))) return;
                        elements.category += `
                            <div class="e_category_btn_for_product buttons category-btn" style="min-height: 100px;">
                ...
                main.data_list.PRODUCTS.forEach(product => {
                    if (product.is_delete === 0){
                        if(self.variable_list.SEARCH_PRODUCT.length > 0) {
                            if(!String(product.name.toLocaleLowerCase("tr")).match(new RegExp(self.variable_list.SEARCH_PRODUCT.toLocaleLowerCase("tr"), "gi"))) return;
                        } else{
                            if(self.variable_list.SELECTED_CATEGORY > 0 && self.variable_list.SELECTED_CATEGORY !== product.category_id){
                                return;
                        ...
                        if(self.variable_list.SELECTED_ORDER_TYPE === helper.db.order_types.SAFE) price = product.price_safe;
                        else if(self.variable_list.SELECTED_ORDER_TYPE === helper.db.order_types.TAKEAWAY) price = product.price_take_away;
                        elements.product += `
                           <div class="e_product p-1 product" product-id="${product.id}">
                                ...
                                <div class="e_product_select product-in btn-s6 p-1 pt-0 pb-1 br text-center" function="add">
                                    ${(self.variable_list.IMAGE_ENABLE === true) ? `<div class="image"> 
                                                <img draggable="false" class="br" src="${(server.is_valid_url(product.image)) ? product.image : settings.paths.image.PRODUCT(main.data_list.BRANCH_ID) + product.image}" alt="${product.name}" /> 
                                           </div>` : ""}
                                    <div class="title fw-6"> <p class="text-c6 bg-none p-0 m-0 btn-title">${product.name}</p> </div>
                                    <div class="price fw-6 text-c3"> ${price.toFixed(2)+main.data_list.CURRENCY} </div>
                                </div>
                           </div>
                        `;
                    }
        ...
        get_order_product_data: function (get_type){
            ...
            switch (get_type) {
                case self.order_product_get_types.ALL:break;
                case self.order_product_get_types.CONFIRMED:break;
        ...
        get_payment: function (get_type) {
            ...
            function create_element_payments(){
                ...
                let payments = array_list.find_multi(main.data_list.PAYMENTS, self.variable_list.SELECTED_ORDER_ID, "order_id");
                payments.forEach(payment => {
            ...
            function create_element_products(){
                ...
                let selected_products = Array.from($(self.class_list.PAYMENT_MODAL_PAYMENT_PRODUCT_SELECTED));
                let orders = array_list.find_multi(main.data_list.ORDERS, self.variable_list.SELECTED_ORDER_ID, "id");
                orders.forEach(order => {
                    let order_products = array_list.find_multi(main.data_list.ORDER_PRODUCTS, order.id, "order_id");
                    if(order_products.length > 0){
                        order_products.forEach(order_product => {
                            ...
                            selected_products.forEach(selected_product => {
                                ...
                                if(selected_product.attr("order-product-id") == order_product.id) is_available = true;
                            });
                            if(is_available) return;
                            ...
                            let price = parseFloat(order_product.price);
                            let option_icon = "";
                            if(order_product_options.length > 0 || order_product.comment !== "" || order_product.quantity != 1){option_icon = "<i class='fa fa-bars'></i>";}
                            if(order_product.status === helper.db.order_product_status_types.CATERING){price = 0;}
                            element += `
            ...
            switch (get_type){
                ...
                case self.get_payment_get_types.PAYMENT_TYPES:
                    $(self.class_list.PAYMENT_MODAL_PAYMENT_TYPES_CUSTOM).html(create_element_payment_types());
                    break;
                ...
                case self.get_payment_get_types.TOTALS:
                    ...
                    `).html($(`${self.id_list.ORDER_LIST} ${self.class_list.ORDER_BODY}[order-id=${self.variable_list.SELECTED_ORDER_ID}] [function='price']`).html());
                    $(`
        ...
        order_combining: function () {
            ...
            Swal.fire({
                ...
                text: `${language.data.ALL_ADDITION}'${self.variable_list.SELECTED_ORDER_ID}'${language.data.ADDITION_QUESTION}`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.value) {
                    set(set_types.ORDER_COMBINING,
                        ...
                        function (data){
                            ...
                            if(data.status) {
                                main.get_order_related_things(main.get_type_for_order_related_things.ORDER_AND_ORDER_PRODUCTS);
                                helper_sweet_alert.success(language.data.PROCESS_SUCCESS_TITLE, language.data.ADDITION_UPDATED);
                                self.get();
                            }
                        }
                    );
        ...
        order_create_element: function (id, options){
            ...
            let price_element = $(`${self.class_list.ORDER_TITLE}[order-id='${self.variable_list.SELECTED_ORDER_ID}'] [function='price']`);
            ...
            return `
                ${($(self.class_list.PRODUCT_ORDER_UNCONFIRMED).length < 1 && self.variable_list.SELECTED_ORDER_ID === 0) ? `
                    <tr class="e_order_title selected" order-id="0" style="background: #0d4197">
                        <td class="e_order_table_hide_column" style="display: none;"></td>
                        <td colspan="3"><span>Yeni Sipariş</span> 
                            <span>Toplam: <span function="price">${(price).toFixed(2)}</span>${main.data_list.CURRENCY}</span>
                        </td>
                    </tr>
                ` : ``}
                <tr class="e_product_order e_product_order_unconfirmed unconfirmed" 
                    ...
                    <td function="name">${option_icon} ${product.name}</td>
                    <td><span function="price" class="text-c5">${(price).toFixed(2)}</span>${main.data_list.CURRENCY}</td> 
                </tr>
            `;
        ...
        add_order_product: function (product = {quantity:1, qty:1}) {
            ...
            let selected_order_id =  (self.variable_list.SELECTED_ORDER_ID > 0 )
                ? ` ${self.class_list.ORDER_BODY + `[order-id=${self.variable_list.SELECTED_ORDER_ID}]`}` : "";

            $( self.id_list.ORDER_LIST + selected_order_id).append(self.order_create_element(self.variable_list.SELECTED_PRODUCT_ID, get_data()));
        ...
        initialize: function () {
            ...
            function set_events(){
                ...
                //Category Click Btn
                $(document).on("click",`${self.class_list.CATEGORIES_LIST} button, ${self.class_list.CATEGORY_BTN_FOR_PRODUCT} button`,function (){
                    let element = $(this);
                    self.variable_list.SELECTED_CATEGORY = parseInt(element.attr("category-id"));
                    self.get_product();
                });

                $(document).on("click",`${self.id_list.PRODUCT_LIST} ${self.class_list.PRODUCT} ${self.class_list.PRODUCT_SELECT}`,function (){
                    let element = $(this);
                    ...
                    switch (function_name) {
                        case "add":
                            ...
                            function get_data(){
                                ...
                                let linked_options = array_list.find_multi(main.data_list.PRODUCT_LINKED_OPTIONS, id, "product_id");
                                linked_options.forEach(linked_option => {
                                    ...
                                    options.forEach(option => {
                                        ...
                                        let option_items = array_list.find_multi(main.data_list.PRODUCT_OPTIONS_ITEMS, option_id, "option_id");
                            ...
                            $( self.id_list.ORDER_LIST + selected_order_id).append(self.order_create_element(id, get_data()));
                            self.order_list_scroll_down();
                            break;
                        case "edit":
                ...
                $(self.id_list.MODAL_PRODUCT_DETAILS).on("show.bs.modal", function () {
                    function create_element(){
                        ...
                        let linked_options = array_list.find_multi(main.data_list.PRODUCT_LINKED_OPTIONS, self.variable_list.SELECTED_PRODUCT_ID, "product_id");
                        linked_options.forEach(linked_option => {
                            let options = array_list.find_multi(main.data_list.PRODUCT_OPTIONS, linked_option.option_id, "id");
                            options.forEach(option => {
                                let option_items = array_list.find_multi(main.data_list.PRODUCT_OPTIONS_ITEMS, option.id, "option_id");
                                elements.options += `
                                    <div class="option" option-type="${option.type}" option-limit="${linked_option.max_count}" option-id="${option.id}">
                                        <p class="mb-1 pt-1">${option.name} ${(linked_option.max_count > 0) ? `<b>Limit: <span class="e_option_count">${linked_option.max_count}</span></b>` : ``}</p>
                                `;
                                option_items.forEach(option_item => {
                                    elements.options += `
                                        <button type="button" class="btn bg-c1 ${(option_item.is_default == 1) ? `selected` : ``}" item-id="${option_item.id}" price="${option_item.price}">${option_item.name} ${(option_item.price != 0) ? `[` + ((option_item.price > 0) ? `+` : ``) + `${option_item.price}]` : `` }</button>
                                    `;
                                });
                                elements.options += `
                                    </div>
                        ...
                        let quantity = array_list.find(main.data_list.PRODUCT_QUANTITY_TYPES, product.quantity_id, "id");

                        elements.quantity = (quantity.id > 1) ? `
                ...
                $(document).on("click", `${self.class_list.OPTIONS} button`, function (){
                    ...
                    if(selected){
                        (element.hasClass("selected")) ? element.removeClass("selected") : element.addClass("selected");
                    }
                ...
                $(document).on("click", `${self.class_list.PRODUCT_ORDER_UNCONFIRMED} td button[function='delete']`, function (e) {
                    let element = $(this);
                    ...
                    if(self.variable_list.SELECTED_ORDER_ID === 0 && $(`${self.class_list.PRODUCT_ORDER_UNCONFIRMED}`).length < 1){
                        $(`tr${self.class_list.ORDER_TITLE}`).remove();
                    }
                    e.stopPropagation()
                })
                    ...
                    switch (function_name){
                            ...
                            if($(self.class_list.PRODUCT_ORDER_UNCONFIRMED).length > 0){
                                set(
                                    ...
                                    function(data){
                                        ...
                                        if(data.status){
                                            main.get_order_related_things(main.get_type_for_order_related_things.ORDER_AND_ORDER_PRODUCTS);
                                            helper_sweet_alert.success(language.data.PROCESS_SUCCESS_TITLE, language.data.PRODUCT_SENT_SUCCESS);
                        ...
                        case "move_table":
                            if($(self.class_list.PRODUCT_ORDER_CONFIRMED).length > 0){
                                if(
                                    self.variable_list.SELECTED_ORDER_ID !== 0 &&
                                    array_list.find_multi(main.data_list.PAYMENTS, self.variable_list.SELECTED_ORDER_ID, "order_id").length > 0
                                ) return;
                                ...
                                $(`${self.id_list.MODAL_PAYMENT_TYPES} [function='price']`).html(
                                    (
                                        (self.variable_list.SELECTED_ORDER_ID === 0)
                                            ...
                                            : $(`${self.id_list.ORDER_LIST} ${self.class_list.ORDER_BODY}[order-id=${self.variable_list.SELECTED_ORDER_ID}] [function='price']`).html()
                                    )
                        ...
                        case "separate_product":
                            let modal_id, discount = `:not([type=${helper.db.order_product_types.DISCOUNT}])`, order_unconfirmed = `:not(${self.class_list.PRODUCT_ORDER_UNCONFIRMED})`, separate_product_one_qty = ``;
                            ...
                            let element_cancel_btn = $(`${self.class_list.ORDER_BTN}[function='${function_name}_cancel']`);
                            if(
                                (   self.variable_list.SELECTED_ORDER_TYPE !== helper.db.order_types.SAFE &&
                                    self.variable_list.SELECTED_ORDER_TYPE !== helper.db.order_types.TAKEAWAY
                                ) &&
                                ...
                                let selected_order_id = ` ${self.class_list.ORDER_BODY + `[order-id=${self.variable_list.SELECTED_ORDER_ID}]`}`;
                                $(`${self.id_list.ORDER_LIST + selected_order_id} ${self.class_list.PRODUCT_ORDER + separate_product_one_qty + order_unconfirmed + discount} ${self.class_list.TABLE_HIDE_COLUMN}`).html(`
                                ...
                                $(`${self.class_list.ORDER_BTN}:not([disable-group='${function_name}'])`).addClass("disable");
                                element_cancel_btn.show();
                            }else {
                                $(`${self.class_list.PRODUCT_ORDER_UNCONFIRMED} td div[function='${function_name}'] div input[type='checkbox']:checked`).closest(self.class_list.PRODUCT_ORDER).remove();
                                if(
                                    (
                                        self.variable_list.SELECTED_ORDER_TYPE === helper.db.order_types.SAFE ||
                                        self.variable_list.SELECTED_ORDER_TYPE === helper.db.order_types.TAKEAWAY
                                    ) ||
                                    $(`${self.class_list.PRODUCT_ORDER} td div[function='${function_name}'] div input[type='checkbox']:checked`).length > 0
                                ){
                                    $(modal_id).modal();
                                }
                            }
                        ...
                        case "separate_product_cancel":
                            ...
                            $(`${self.id_list.ORDER_LIST} ${self.class_list.PRODUCT_ORDER} ${self.class_list.TABLE_HIDE_COLUMN}`).html("");
                            $(`${self.class_list.ORDER_BTN}`).removeClass("disable");
                            break;
                        case "discount":
                            if($(self.class_list.PRODUCT_ORDER_CONFIRMED).length > 0 && self.variable_list.SELECTED_ORDER_ID > 0){
                                $(self.id_list.FORM_MODAL_DISCOUNT).trigger("reset");
                                $(`${self.id_list.FORM_MODAL_DISCOUNT} [name='type'][value='1']`).trigger("click").change();
                                $(self.id_list.MODAL_DISCOUNT).modal();
                            }
                            break;
                        case "payment":
                            if($(self.class_list.PRODUCT_ORDER_UNCONFIRMED).length < 1 && $(self.class_list.PRODUCT_ORDER_CONFIRMED).length > 0 && self.variable_list.SELECTED_ORDER_ID > 0) {
                        ...
                        case "print_safe":
                            ...
                            }else if(self.variable_list.SELECTED_TABLE_ID === helper.db.branch_tables.TAKE_AWAY) {
                                invoice.payment_receipt_takeaway(self.variable_list.SELECTED_ORDER_ID,self.variable_list.SELECT_ADDRESS_STRING)
                            }else {
                        ...
                        case "read_barcode":
                            ...
                            $(`${self.id_list.MODAL_BARCODE_SYSTEM} input${self.class_list.BARCODE_DETAILS}`).focus();
                            break;
                        case "change_status":
                ...
                $(document).on("click", self.class_list.PRODUCT_ORDER, function () {
                    let element = $(this);
                    let element_delete_checkbox = element.children("td").children("div[function='delete_product'],[function='catering_product'],[function='change_price'],[function='separate_product']").children("div").children("input[type='checkbox'],input[type='radio']");
                    if(element_delete_checkbox.length > 0){
                        element_delete_checkbox.prop("checked", !element_delete_checkbox.prop("checked")).change();
                    }else {
                        ...
                        function create_element(){
                            ...
                            if(quantity !== 1){
                                let product_quantity = array_list.find(main.data_list.PRODUCT_QUANTITY_TYPES, product.quantity_id, "id");
                                element += `
                            ...
                            `;
                                order_product_options.forEach(order_product_option => {
                                    let option = array_list.find(main.data_list.PRODUCT_OPTIONS, order_product_option.option_id, "id");
                                    let option_item = array_list.find(main.data_list.PRODUCT_OPTIONS_ITEMS, order_product_option.option_item_id, "id");
                                    element += `
                                    <li>[${option.name}]-> ${order_product_option.qty}x ${option_item.name} ${(order_product_option.price != 0) ? `[${((order_product_option.price > 0) ? `+` : ``)}${(order_product_option.price) + main.data_list.CURRENCY}]` : ``}</li>
                                `;
                                });
                                element += `
                                    </ul>
                ...
                $(`${self.id_list.MODAL_DELETE_PRODUCT},${self.id_list.MODAL_CATERING_PRODUCT},${self.id_list.MODAL_CHANGE_PRICE},${self.id_list.MODAL_SEPARATE_PRODUCT}`).on("show.bs.modal", function () {
                    let id = "#"+$(this).attr("id");
                    let attr_value,form_id,products_id,readonly_qty = "", readonly_price = "";
                    ...
                    switch (id) {
                        ...
                        case self.id_list.MODAL_CATERING_PRODUCT:
                            ...
                            $(`${self.id_list.FORM_MODAL_CATERING} select[name='owner']`).html(helper.get_select_options(main.data_list.CATERING_OWNERS, "id", "name"));
                            $(`${self.id_list.FORM_MODAL_CATERING} select[name='question']`).html(helper.get_select_options(main.data_list.CATERING_QUESTIONS, "id", "comment"));
                            readonly_price = "readonly";
                            break;
                    ...
                    function create_element(){
                        ...
                        products.forEach(product => {
                            ...
                            elements += `
                                <tr row="${product.attr("row")}" product-id="${product.attr("product-id")}" order-product-id="${product.attr("order-product-id")}" options ='${product.attr("options")}' >
                                    <td function="qty" old-qty="${product.attr("qty")}">
                                        <input class="form-input" type="number" name="qty" value="${product.attr("qty")}" max="${product.attr("qty")}" min="1" ${readonly_qty} required>
                                    </td>
                                    <td>
                                        ${product.children("td[function='name']").html()}
                                    </td>
                                    ${(attr_value !== "separate_product")
                                ? `<td function="price" old-price="${product.attr("price")}">
                                        <input class="form-input" type="number" name="price" step="0.01" min="0" max="9999999999" value="${product.attr("price")}" ${readonly_price} required>
                                   </td>`
                                : `<td function="qty_separate">
                                        <input class="form-input" type="number" name="qty_separate" min="1"  max="${product.attr("qty") - 1}" value="${product.attr("qty") - 1}" required>
                                    </td>
                                    <td function="piece_separate">
                                        <input class="form-input" type="number" name="piece_separate" min="1"  max="${product.attr("qty") - 1}" value="1" required>
                                    </td>
                                    `
                ...
                $(document).on(
                    ...
                    function () {
                        ...
                        let new_qty = parseInt(element.children("td[function='qty']").children("input[name='qty']").val());
                ...
                $(`${self.id_list.FORM_MODAL_DELETE},${self.id_list.FORM_MODAL_CATERING},${self.id_list.FORM_MODAL_CHANGE_PRICE},${self.id_list.FORM_MODAL_SEPARATE_PRODUCT}`).submit(function (e) {
                    e.preventDefault();
                    let id = "#"+$(this).attr("id");
                    let attr_value,modal_id,products_id,function_type,data = {},set_type = set_types.CANCEL_AND_CATERING;
                    ...
                    switch (id) {
                        ...
                        case self.id_list.FORM_MODAL_CHANGE_PRICE:
                            ...
                            products_id = self.class_list.CHANGE_PRICE_PRODUCTS;
                            modal_id = self.id_list.MODAL_CHANGE_PRICE;
                            function_type = parseInt($(`${self.id_list.FORM_MODAL_CHANGE_PRICE} button[type=submit]:focus`).attr("change-id"));
                    ...
                    function get_data(){
                        let order = array_list.find(main.data_list.ORDERS, self.variable_list.SELECTED_ORDER_ID,"id");
                        data = Object.assign(
                            data,
                            {
                                "order_id": self.variable_list.SELECTED_ORDER_ID,
                        ...
                        let products = Array.from($(`${products_id} tr`));
                            ...
                            data.products.push({
                                ...
                                "price": product.children("td[function='price']").children("input[name='price']").val(),
                                "qty_separate": product.children("td[function='qty_separate']").children("input[name='qty_separate']").val(),
                                "piece_separate": product.children("td[function='piece_separate']").children("input[name='piece_separate']").val(),
                            });
                        });
                    ...
                    if(
                        ...
                        data.products.forEach(product => {
                            ...
                            let find_product = array_list.find(main.data_list.PRODUCTS, parseInt(element_product.attr("product-id")), "id");
                            ...
                            element_product.children("td").children("span[function='price']").html(price.toFixed(2));
                        });
                        ...
                        set(
                            ...
                            function (data) {
                                data = JSON.parse(data);
                                main.get_order_related_things(main.get_type_for_order_related_things.ORDER_AND_ORDER_PRODUCTS);
                                $(`${self.class_list.ORDER_BTN}[function='${attr_value}_product_cancel']`).trigger("click");
                                $(modal_id).modal("toggle");
                                helper_sweet_alert.success(language.data.PROCESS_SUCCESS_TITLE, language.data.PROCESS_SUCCESS);
                                self.get();
                            }
                ...
                $(document).on("click", self.class_list.ORDER_TITLE, function () {
                    ...
                    if(self.variable_list.SELECTED_ORDER_ID === id && self.variable_list.SELECTED_ORDER_ID !== 0){
                        self.variable_list.SELECTED_ORDER_ID = 0;
                ...
                $(document).on("click", `${self.class_list.PAYMENT_TYPES} ${self.class_list.PAYMENT_BTN}, ${self.class_list.PAYMENT_MODAL_PAYMENT_BTN}`,function () {
                    ...
                    function get_data() {
                        ...
                        switch(self.variable_list.SELECTED_PAYMENT_MODE){
                            case self.payment_modes.FAST:
                                let attr_order_id = (self.variable_list.SELECTED_ORDER_ID > 0) ? `[order-id=${self.variable_list.SELECTED_ORDER_ID}]` : ``;
                            ...
                            case self.payment_modes.NORMAL:
                                let products = Array.from($(self.class_list.PAYMENT_MODAL_PAYMENT_PRODUCT_SELECTED));
                                products.forEach(product => {
                                    product = $(product);
                                ...
                                data.orders.push({
                                    ...
                                    "price": parseFloat($(`${self.id_list.MODAL_PAYMENT} [function='payment_price'] [function='price']`).html())
                                });
                                break;
                        }
                    ...
                    switch(self.variable_list.SELECTED_PAYMENT_MODE){
                        case self.payment_modes.FAST:
                            set(
                                ...
                                function(){
                                    ...
                                    self.get();
                                    if($(self.class_list.PRODUCT_ORDER_CONFIRMED).length < 1) self.back_detail(self.back_detail_types.TABLE);
                        ...
                        case self.payment_modes.NORMAL:
                            Swal.fire({
                                ...
                                html: `<b>'${array_list.find(main.data_list.PAYMENT_TYPES, id, "id").name}'</b> ${language.data.PAYMENT_QUESTION}?`,
                                allowEscapeKey: false,
                                allowOutsideClick: false,
                                showCancelButton: true,
                                confirmButtonText: language.data.ACCEPT,
                                ...
                                if (result.value) {
                                    set(
                                        ...
                                        function(data){
                                            ...
                                            helper_sweet_alert.success(language.data.PROCESS_SUCCESS_TITLE, language.data.PAID_SUCCESS_TEXT);
                                            self.variable_list.SELECTED_ORDER_ID = (typeof data.custom_data.new_order_id !== "undefined") ? parseInt(data.custom_data.new_order_id) : self.variable_list.SELECTED_ORDER_ID;
                                            self.variable_list.SELECTED_TRUST_ACCOUNT_ID = 0;
                                            self.get();
                                            if(parseFloat($(`${self.id_list.MODAL_PAYMENT} [function='total_price'] [function='price']`).html()) <= parseFloat($(`${self.id_list.MODAL_PAYMENT} [function='payment_price'] [function='price']`).html())){
                                            ...
                                            self.variable_list.SELECTED_ORDER_ID = ($(`${self.class_list.ORDER_BODY}[order-id='${self.variable_list.SELECTED_ORDER_ID}']`).length < 1) ? 0 : self.variable_list.SELECTED_ORDER_ID;
                ...
                $(document).on("click", `${self.class_list.TRUST_ACCOUNTS} tr[account-id]`, function () {
                    ...
                    Swal.fire({
                        ...
                        html: `<b>'${array_list.find(main.data_list.TRUST_ACCOUNTS, self.variable_list.SELECTED_TRUST_ACCOUNT_ID, "id").name}'</b>${language.data.ACCOUNT_CREDIT_QUESTION}`,
                        allowEscapeKey: false,
                        allowOutsideClick: false,
                        showCancelButton: true,
                        confirmButtonText: language.data.ACCEPT,
                        ...
                        if (result.value) {
                            ...
                            $(`${self.class_list.PAYMENT_TYPES} ${self.class_list.PAYMENT_BTN}[type-id='6']`).trigger("click");
                        }
                    });
                });
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if(data.status){
                                ...
                                $(this).trigger("reset");
                                helper_sweet_alert.success(language.data.PROCESS_WAIT_CONTENT, language.data.PROCESS_SUCCESS);
                            }
                ...
                $(self.id_list.FORM_MODAL_DISCOUNT).submit(function (e) {
                    ...
                    function get_data(){
                        let data = {
                            ...
                            "table_id": self.variable_list.SELECTED_TABLE_ID,
                            "order_id": self.variable_list.SELECTED_ORDER_ID,
                            "no": array_list.find(main.data_list.ORDERS,self.variable_list.SELECTED_ORDER_ID,"id").no,
                            "discount": 0,
                            "type": self.variable_list.SELECTED_ORDER_TYPE,
                            "status": helper.db.order_status_types.GETTING_READY
                        }

                        let price = $(`${self.id_list.ORDER_LIST} ${self.class_list.ORDER_BODY}[order-id=${self.variable_list.SELECTED_ORDER_ID}] [function='price']`).html();
                        price = parseFloat(price);

                        let discount = (form_data.type == self.discount_types.PRICE)
                            ? form_data.price
                    ...
                    set(
                        ...
                        function(data){
                            ...
                            if(data.status){
                                main.get_order_related_things(main.get_type_for_order_related_things.ORDER_AND_ORDER_PRODUCTS);
                                helper_sweet_alert.success(language.data.PROCESS_WAIT_CONTENT, language.data.ADDED_DISCOUNT);
                                self.get();
                                self.get_payment(self.get_payment_get_types.ALL);
                ...
                $(document).on("click", self.class_list.PAYMENT_MODAL_PRODUCT, function () {
                    ...
                    function create_element(){
                        let option_icon = "";
                        if(JSON.parse(element.attr("options")).length > 0 || element.attr("comment") !== "" || element.attr("quantity") != 1){ option_icon = "<i class='fa fa-bars'></i>"; }
                        return `
                    ...
                    $(this).remove();
                    let element_total_price = $(`${self.id_list.MODAL_PAYMENT} [function='total_price'] [function='price']`);
                    let price = ($(self.class_list.PAYMENT_MODAL_PAYMENT_PRODUCT_SELECTED).length > 0) ? parseFloat(element_total_price.html()) + parseFloat(element.attr("price")) : parseFloat(element.attr("price"));
                    element_total_price.html(price.toFixed(2));
                    $(`${self.id_list.MODAL_PAYMENT} [function='payment_price'] [function='price']`).html(price.toFixed(2));
                    $(self.class_list.PAYMENT_MODAL_PAYMENT_PRODUCTS_SELECTED).append(create_element());
                });

                $(document).on("click", self.class_list.PAYMENT_MODAL_PAYMENT_PRODUCT_SELECTED, function () {
                    let element = $(this);
                    let element_total_price = $(`${self.id_list.MODAL_PAYMENT} [function='total_price'] [function='price']`);
                    let price = ($(self.class_list.PAYMENT_MODAL_PAYMENT_PRODUCT_SELECTED).length > 0) ? parseFloat(element_total_price.html()) - parseFloat(element.attr("price")) : 0;
                    element_total_price.html(price.toFixed(2));
                    $(`${self.id_list.MODAL_PAYMENT} [function='payment_price'] [function='price']`).html(price.toFixed(2));
                    $(this).remove();

                    self.get_payment((price === 0) ? self.get_payment_get_types.ALL : self.get_payment_get_types.PRODUCTS);
                });

                $(`${self.class_list.PAYMENT_MODAL_CALCULATOR} button`).on("click", async function () {
                    let value = parseInt($(this).attr("value"));
                    let element = $(`${self.id_list.MODAL_PAYMENT} [function='payment_price'] [function='price']`);
                    let price_total = parseFloat($(`${self.id_list.MODAL_PAYMENT} [function='total_price'] [function='price']`).html());
                    ...
                    switch (value) {
                        ...
                        case -7:
                            ...
                            if(value_round !== 0){
                                $(`${self.id_list.FORM_MODAL_DISCOUNT} input[name="comment"]`).val("Yuvarlama Yapıldı!");
                                $(`${self.id_list.FORM_MODAL_DISCOUNT} input[name="price"]`).val(value_round);
                                $(`${self.id_list.FORM_MODAL_DISCOUNT} input[name="type"][value="1"]`).prop("checked", true);
                    ...
                    if(change_price < 0) {
                        ...
                        $(`${self.id_list.MODAL_PAYMENT} [function='change_price'] [function='price']`).html((change_price * -1).toFixed(2));
                    }else{
                        $(self.class_list.PAYMENT_MODAL_CHANGE_PRICE).hide();
                    }
                ...
                $(document).on("click",`button.e_takeaway_confirm`,function (){
                    ...
                    let input =  (confirm === 1)  ? "" : `<input type="text" class="form-input e_take_away_cancel_comment" placeholder="İptal Nedeni">`;
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            if(self.variable_list.SELECTED_TABLE_TYPE === self.variable_list.table_types.DEFAULT) {
                                set(set_types.UPDATE_CONFIRM,{order_id: self.variable_list.SELECTED_ORDER_ID,confirm: confirm},function () {
                                ...
                                switch (integrations.variable_list.SELECTED_INTEGRATE_TYPE){
                                    case helper.db.integrate_types.YEMEK_SEPETI:
                                        ...
                                        if(confirm === 1){
                                            ...
                                            let data_order = integrations.yemek_sepeti.variable_list.DATA[integrations.yemek_sepeti.variable_list.SELECTED_DATA_INDEX];
                                            ...
                                            data_order.product.forEach(product => {
                                                ...
                                                let product_integrated = array_list.find(main.data_list.PRODUCTS_INTEGRATE, product.id, "product_id_integrated");
                                                if(typeof product_integrated === "undefined"){
                                                    is_all_matching = false;
                                                    return;
                                                }
                                                let product_current = array_list.find(main.data_list.PRODUCTS, product_integrated.product_id, "id");

                                                let data_options = Array();
                                                options.forEach(option => {
                                                    if(!is_all_matching) return;
                                                    option = option["@attributes"];
                                                    let option_integrated = array_list.find(main.data_list.PRODUCT_OPTIONS_INTEGRATE, option.Id, "option_id_integrated")
                                                    if(typeof option_integrated === "undefined"){
                                                        is_all_matching = false;
                                                        return;
                                                    }
                                                    let option_current = array_list.find(main.data_list.PRODUCT_OPTIONS_ITEMS, option_integrated.option_id, "id");
                                                    data_options.push({
                                            ...
                                            if(!is_all_matching) {
                                                helper_sweet_alert.error("İşlem Onaylanamadı!", "Lütfen tüm ürün ve opsiyonları eşleştirdiğinizden emin olunuz!");
                                                return false;
                                            }

                                            data_order = data_order["@attributes"];
                                            let integrate_payment_id = array_list.find(main.data_list.PAYMENT_TYPES_INTEGRATE, data_order.PaymentMethodId, "type_id_integrate");
                                            integrate_payment_id = (typeof integrate_payment_id === "undefined") ? `-${data_order.PaymentMethodId}` : integrate_payment_id.id;
                                            ...
                                            let data_insert = {
                                                ...
                                                "address_integrate":
                                                    ...
                                                    + `${((data_order.CustomerPhone2 !== "") ? ` | Telefon Numarası - 2: ${data_order.CustomerPhone2}` : ``)}`
                                                    + `${((data_order.OrderNote !== "") ? ` | Sipariş Notu: ${data_order.OrderNote}` : ``)}`
                                            ...
                                            set(
                                                ...
                                                function (data) {
                                                    ...
                                                    console.log(data);
                                                    main.get_order_related_things(main.get_type_for_order_related_things.ORDER_AND_ORDER_PRODUCTS);
                                                    main.get_integrate_related_things(main.get_type_for_integrate_related_things.ORDERS);
                                                }
                                            );
                                        }
                                        ...
                                        integrated_companies.yemek_sepeti.orders.set_message_successful(
                                            ...
                                            function (data) {
                                                if(data.rows.status){
                                                    let state =  (confirm === 1)  ? integrated_companies.yemek_sepeti.orders.variable_list.order_state.enums.ACCEPTED : integrated_companies.yemek_sepeti.orders.variable_list.order_state.enums.CANCELLED;
                                ...
                                self.back_detail(self.back_detail_types.TABLE);
                                $(`${integrations.class_list.TABLE}[integrate-type="${integrations.variable_list.SELECTED_INTEGRATE_TYPE}"][order-id="${self.variable_list.SELECTED_ORDER_ID}"]`).remove();
                            }
                        }
                    });
                })
                ...
                //barcode
                $(document).on("keypress",`${self.id_list.MODAL_BARCODE_SYSTEM} ${self.class_list.BARCODE_INPUT}`,function(e){
                    const kg = 1000;
                    const keycode = {
                        ENTER: 13
                    }
                    if(e.which === keycode.ENTER) {
                        let html = "";
                        let numbers = $(`${self.id_list.MODAL_BARCODE_SYSTEM} ${self.class_list.BARCODE_INPUT}`).val()
                        ...
                        if(numbers.length === 13) {
                            ...
                            if (typeof p == "undefined") {
                                ...
                                $(`${self.id_list.MODAL_BARCODE_SYSTEM} .e_message`).addClass("alert-danger").addClass("alert-success").html("<h2 style='color: red' class='p-0 m-0 text-center'>ÜRÜN BULUNAMADI !</h2>")
                                return false;
                            }
                        ...
                        $(`${self.id_list.MODAL_BARCODE_SYSTEM} ${self.class_list.BARCODE_DETAILS}`).append(html);
                        $(`${self.id_list.MODAL_BARCODE_SYSTEM} .e_message`).removeClass("alert-danger").addClass("alert-success").html("<h2 class='m-0 p-0 text-center' style='color: #0c0c0c'>Ürün Eklendi</h2>")
                        $(`${self.id_list.MODAL_BARCODE_SYSTEM} ${self.class_list.BARCODE_INPUT}`).val("")
                    }
                })
                ...
                $(document).on("click", self.class_list.BUTTON_ORDER_STATUS_TYPE, function () {
                    ...
                    switch (integrations.variable_list.SELECTED_INTEGRATE_TYPE) {
                        case helper.db.integrate_types.YEMEK_SEPETI:
                            if(
                                type === integrated_companies.yemek_sepeti.orders.variable_list.order_state.enums.CANCELLED ||
                                type === integrated_companies.yemek_sepeti.orders.variable_list.order_state.enums.REJECTED
                            ) confirm = false;
                            break;
                    }
                    ...
                    let input =  (confirm)  ? "" : `<input type="text" class="form-input e_take_away_cancel_comment" placeholder="İptal Nedeni">`;
                            ...
                            switch (integrations.variable_list.SELECTED_INTEGRATE_TYPE) {
                                case helper.db.integrate_types.YEMEK_SEPETI:
                                    integrated_companies.yemek_sepeti.orders.set_status(
                                        ...
                                        reason,
                                        function (data) {
                                            helper_sweet_alert.success("İşlem Başarılı", `Sipariş  durumu <b>'${type_name}'</b> olarak güncellenmiştir.`);
                                        }
                                    );
                                    break;
                            }
    ...
    let integrations = {
        ...
        yemek_sepeti: {
            ...
            start: function () {
                let self = this;
                self.variable_list.INTERVAL = setInterval(() => { self.get_orders(); }, settings.ajax_timeouts.NORMAL)
            },
            stop: function () {
                let self = this;
            ...
            get_order_products: function () {
                ...
                let element_html = $(`${integrations.id_list.TABLE_GROUP} ${self.class_list.TABLE_GROUP} [order-id="${table_detail.variable_list.SELECTED_ORDER_ID}"]`);
                ...
                data_product.forEach(product => {
                    ...
                    product = product["@attributes"];
                    let product_integrated = array_list.find(main.data_list.PRODUCTS_INTEGRATE, product.id, "product_id_integrated");
                    if(typeof product_integrated === "undefined") custom_class = "order-product-integrate-not-matching";
                    element += `
                    ...
                    <tbody class="e_order_body" style="display: contents;">
                        <tr class="e_order_title selected" style="background: #0d4197">
                            <td colspan="3">
                                ...
                                <span>No: ${data_order.Id}</span> <span>Toplam: <span>${data_order.OrderTotal.toFixed(2)}</span>${main.data_list.CURRENCY}</span>
                            </td>
                        </tr>
                        ${element}
                    </tbody>
            ...
            get_elements_table: function () {
                ...
                function create_element(){
                    ...
                    self.variable_list.DATA.forEach((data, index) => {
                        ...
                        table-id="${helper.db.branch_tables.YEMEK_SEPETI}"
                        class="e_table_integrations order-table order-table-xl ${table_list.settings.type[0]} p-0">
                            <div class="in ${table_list.settings.order_type[1]} yemek-sepeti order-unconfirmed">             
                                <div class="tc-1 w-100 text-center fw-6">${data.Id}</div>
                                <div class="tc-2 w-100 size-type-lg fw-6 text-center">Yemek Sepeti</div>
                                <div class="tc-3 w-100 size-type-lg fw-6 text-center">
                                   <p class="e_last_time mx-0 mb-0 d-block float-left">${time}</p>
                                   <p class="e_last_price mx-0 mb-0 d-block float-left">${data.OrderTotal.toFixed(2) + main.data_list.CURRENCY}</p>
                                </div>
                                <div class="e_total_price tc-4 w-100 size-type-lg fw-6 text-center">${data.OrderTotal.toFixed(2) + main.data_list.CURRENCY}</div>              
                            </div>
                        </div>
                    `;
                    });
                ...
                $(`${integrations.id_list.TABLE_GROUP} ${self.class_list.TABLE_GROUP}`).html(create_element());
            }
        },
        initialize: function () {
            let self = this;
            ...
            function set_events() {
                $(document).on("click", self.class_list.TABLE, function () {
                    ...
                    table_detail.variable_list.SELECTED_TABLE_TYPE = table_detail.variable_list.table_types.INTEGRATION;
                    ...
                    $(`${table_detail.id_list.TABLE_DETAILS} ${table_detail.class_list.TABLE_TITLE}`).html(`${elements.section_name.toUpperCase()} ${elements.table_no}`);
                    $(`${table_detail.id_list.TABLE_DETAILS} ${table_detail.class_list.TABLE_PRICE_TOTAL} [function='price']`).html(`${elements.table_total.toFixed(2)}`);
                    $(`${table_detail.id_list.TABLE_DETAILS} ${table_detail.class_list.TABLE_PRICE_TOTAL} [function='currency']`).html(`${main.data_list.CURRENCY}`);
                    $(table_detail.class_list.TABLE_TITLE_INFO).html(elements.title_info);
                    $(`${table_detail.class_list.TAKE_AWAY_INFO} [function="address"]`).html(elements.address);
                    $(`
                ...
                $(document).on("click", self.class_list.PRODUCTS_INTEGRATE, function () {
                    ...
                    function create_element(){
                        ...
                        options.forEach(data => {
                            let not_matching = "";
                            data = data["@attributes"];
                            let option_integrated = array_list.find(main.data_list.PRODUCT_OPTIONS_INTEGRATE, data.Id, "option_id_integrated");
                            if(typeof option_integrated === "undefined") not_matching = `<b class="order-product-option-integrate-not-matching">(Eşleştirilmemiş Opsiyon!)</b>`;
