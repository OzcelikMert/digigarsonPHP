let page_index = (function () {
  ...
  let check_login = {
    ...
    set: function (return_function = function () {}, un_token = false) {
      ...
      form_data.append("token", app.token);
        ...
        case self.login_types.PASSWORD:
          if (data.status === true) {
            helper_sweet_alert.success(
              language.data.LOGIN_SUCCESS,
              language.data.REDIRECTED
            );
            setTimeout(() => {
    ...
    initialize: function () {
      let self = this;
      helper_sweet_alert.wait(language.data.PAGE_LOADING, language.data.LOADING);

      function set_events() {
        $(document).ready(async function () {
          let token = await app.users.get_token();
          if (token.length > 0) {
