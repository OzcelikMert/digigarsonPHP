  ...
  function Header() {
    ...
    if (data.info.safe !== 0) {
      ...
      return `
            <div class="header">
                <span class="font-size-xxm bold text-center mb-5 w-100 d-block">Z RAPORU</span>
                <span class="font-size-xxs bold text-left w-100 d-block">Z Raporu Tarih: ${new Date().toLocaleDateString(
                  "tr-TR"
                )}</span>
            </div>
            `;
  ...
  function Body() {
      ...
      if (payment.type !== helper.db.payment_types.CANCEL) {
        Body += `
                    <tr>
                        ...
                        <td>
                            <span class="font-size-xs text-left bold text-right w-100 d-block">
                                ${parseFloat(payment.price).toFixed(2)}<span class="lighter">${
          data.info.currency
        }</span>
                            </span>
                        </td>
        ...
        if (payment.type === helper.db.payment_types.CASH) {
          Body += `
                        <tr>
                            ...
                            <td>
                                <span class="font-size-xs text-left bold text-right w-100 d-block">
                                    ${(parseFloat(payment.price) + costs).toFixed(
                                      2
                                    )}<span class="lighter">${data.info.currency}</span>
                                </span>
                            </td>
                        </tr>
                    `;
    ...
    Body += `
        <tr>
            ...
            <td><span class="font-size-xs text-left bold text-right w-100 d-block"> ${parseFloat(
              data.info.cancel_price
            ).toFixed(2)}<span class="lighter">${data.info.currency}</span></span></td>
        </tr>
        
        <tr>
            <td><span class="font-size-xs text-left w-100 d-block">TOPLAM TUTAR</span></td>
            <td><span class="font-size-xs text-left bold text-right w-100 d-block"> ${total_safe_price.toFixed(
              2
            )}<span class="lighter">${data.info.currency}</span></span></td>
        </tr>
    ...
    data.products.forEach(function (product) {
      ...
      Body += `
                <tr>
                    <td><span class="font-size-x text-left w-100 d-block">${
                      product.qty + "x " + product.name.toUpperCase()
                    }</span></td>
                    <td>
                        <span class="font-size-x text-left bold text-right w-100 d-block">
                            ${parseFloat(product.price).toFixed(2)}<span class="lighter">${
        data.info.currency
      }</span>
                        </span>
                    </td>
      ...
      if (product.quantity_id > 1) {
        let quantity_name = array_list.find(
      ...
      product.options.forEach((option) => {
        ...
        Body += `
                    <tr>
                        <td><span class="font-size-xxs text-left ml-2 w-100 d-block">• ${
                          option.qty + `x ` + option.name
                        }</span></td>
                        <td>
                            <span class="font-size-xxs text-left bold text-right w-100 d-block">${parseFloat(
                              option.price
                            ).toFixed(2)}
    ...
    if (data.cancel_products.length > 0) {
