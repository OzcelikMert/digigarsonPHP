...
function set_echo_values(db $db, get $sessions, echo_values &$echo) : void{
    ...
    if(($type == get_types::ALL || $type ==  get_types::PAYMENTS)) $echo->rows["order_payments"] = (array)payments::get($db, $sessions->LANGUAGE_TAG, $sessions->BRANCH_ID, safe_id: 0, order_by: tbl::ID." DESC");
    if(($type == get_types::ALL || $type ==  get_types::STATUS_TYPES)) $echo->rows["order_payment_status_types"] = (array)payments::get_status_types($db, $sessions->LANGUAGE_TAG, order_by: tbl2::ID." asc");
}
