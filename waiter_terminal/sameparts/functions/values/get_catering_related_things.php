...
function set_echo_values(db $db, get $sessions, echo_values &$echo) : void{
    ...
    if(user::post(post_keys::PAGE_NAME) == page_names::POS()::FINANCE){
        ...
        ) $echo->rows["caterings"] = (array)orders::get_caterings($db, $sessions->BRANCH_ID, order_by: $db->order_by(tbl3::ID, $db::DESC));
    }

    if(
        $type ==  get_types::ALL ||
        $type ==  get_types::CATERING_OWNERS
    ) $echo->rows["catering_owners"] = (array)orders::get_catering_owners($db, $sessions->BRANCH_ID, order_by: $db->order_by(tbl::ID, $db::DESC));
    if(
        $type ==  get_types::ALL ||
        $type ==  get_types::CATERING_QUESTIONS
    ) $echo->rows["catering_questions"] = (array)orders::get_catering_questions($db, $sessions->BRANCH_ID, order_by: $db->order_by(tbl2::ID, $db::DESC));
}
