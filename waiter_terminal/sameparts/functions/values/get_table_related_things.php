...
function set_echo_values(db $db, get $sessions, echo_values &$echo) : void{
    ...
    if(($type == get_types::ALL || $type ==  get_types::TABLES)) $echo->rows["tables"] = (array)branch_tables::get($db, $sessions->BRANCH_ID, order_by:  $db->order_by_multi([
        $db->order_by($db->length(tbl::NO), db::ASC),
        $db->order_by(tbl::NO, db::ASC)
    ]));
    if(($type == get_types::ALL || $type ==  get_types::SECTIONS)) $echo->rows["sections"] = (array)branch_tables::get_sections($db, $sessions->BRANCH_ID, group_by: tbl2::ID.",".tbl2::SECTION_ID, order_by: tbl2::RANK." asc");
    if(($type == get_types::ALL || $type ==  get_types::SECTION_TYPES)) $echo->rows["section_types"] = (array)branch_tables::get_section_types($db, $sessions->LANGUAGE_TAG, order_by: tbl3::ID." desc");
}
