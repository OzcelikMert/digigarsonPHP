let page_products = (function() {
    let default_ajax_path = `../pos/${settings.paths.primary.PHP}products/`;
    let get_types = {
                   ...
                   data = JSON.parse(data);
                    ...
                    switch (data.error_code){
                        ...
                        case settings.error_codes.SUCCESS:
                            helper_sweet_alert.success(language.data.SUCCESS, language.data.CATEGORY_ADD_MESSAGE);
                            break;
                    }
        ...
        create_title: function(cols=Array()){
            return `
                <div class="col-12 row mt-2 m-0">
                    ${(cols[0] !== "") ? `<div class="${cols[0]}"><label class="col-form-label mt-2 pl-1 pr-1">${language.data.DELETE}</label></div>` : "" }
                    ${(cols[1] !== "") ? `<div class="${cols[1]} pl-1 pr-1"><label class="col-form-label mt-2">${language.data.NAME}</label></div>` : "" }
                    ${(cols[2] !== "") ? `<div class="${cols[2]} pl-1 pr-1"><label class="col-form-label mt-2">${language.data.PRICE}</label> </div>` : "" }
                    ${(cols[3] !== "") ? `<div class="${cols[3]} pl-1 pr-1"><label class="col-form-label mt-2">${language.data.QUANTITY}</label></div>` : "" }
                    ${(cols[4] !== "") ? `<div class="${cols[4]} pl-1 pr-1"><label class="col-form-label mt-2">${language.data.DEFAULT}</label></div>` : "" }
                </div>`;
        },
        ...
        get: function(){
            ...
            function create_element(){
                ...
                main.data_list.PRODUCT_OPTIONS.forEach(function (v){
                    let btns = `
                        <button class="btn btn-s1 m-0" style="height: 45px;" function="edit">${language.data.EDIT}</button>
                        <button class="btn btn-s3 m-0" style="height: 45px;" function="del">${language.data.DELETE}</button>`;
                    values +=
                        `<tr option_id="${v.id}">
                            <td>${v.id}</td>
                            <td>${v.search_name}</td>
                    ...
                    switch (function_value) {
                        ...
                        case 2:
                            Swal.fire({
                                ...
                                html: `<b style="color: black">'${array_list.find(main.data_list.PRODUCT_CATEGORIES, parseInt($(`${self.id_list.FORM} input[name="id"]`).val()), "id").name}'</b> ${language.data.DELETE_CATEGORY_HTML}`,
                                allowEscapeKey: false,
                                allowOutsideClick: false,
                                showCancelButton: true,
                                confirmButtonText: language.data.DELETE,
    ...
    let modal_product = {
        ...
        set: function (){
            ...
            console.log(variable.is_base64(image));
            ...
            console.log(`${default_ajax_path}set.php`);
            $.ajax({
                url: `${default_ajax_path}set.php`,
                type: "POST",
                cache: false,
                ...
                success: function (data) {
                    ...
                    page.get();
                    helper_sweet_alert.success(language.data.PROCESS_SUCCESS_TITLE, language.data.PRODUCT_ADD_TEXT);
                },error: helper_sweet_alert.close(), timeout: settings.ajax_timeouts.NORMAL
            });
        },
        clear_form: function (){
            let self = this;
            $(`${self.id_list.MODAL} input[name=id]`).val(0);
                ...
                $("#product_linked_btn").on("click", function() {
                    ...
                    $(document).on("click",`${modal_get_options.id_list.MODAL} ${modal_get_options.class_list.OPTION_LIST} tr`,function (){
                        ...
                        if ($(modal_get_options.id_list.MODAL).attr("linked") === "1"){
                            Swal.fire({
                                icon: "question",
                                title: language.data.PRODUCT_OPTION_QUESTION,
            ...
            function create_element(){
                ...
                main.data_list.PRODUCTS.forEach(product => {
                    ...
                    if(typeof category === "undefined"){
                        category = {
                            id: 0,
                            name: `<font color="red"><lang>${language.data.DELETED}</lang>!</font>`
                        }
                    }
