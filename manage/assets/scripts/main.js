let main = (function () {
    ...
    main.get_branch_related_things = function(get_type = main.get_type_for_branch_related_things.ALL, async = false){
        $.ajax({
    ...
    main.get_order_related_things = function(get_type = main.get_type_for_order_related_things.ALL, safe_id = 0, async = false){
        $.ajax({
    ...
    main.get_integrate_related_things = function(get_type = main.get_type_for_integrate_related_things.ALL, safe_id = 0, async = false){
        $.ajax({
    ...
    main.get_survey_related_things = function(get_type = main.get_type_survey_related_things.ALL, safe_id = 0, async = false){
        $.ajax({
    ...
    function set_data_list(data, set_type){
        switch (set_type){
            ...
            case main.set_type_for_data_list.BRANCH_RELATED_THINGS:
                if(variable.isset(()=> data.rows.branch_id)) main.data_list.BRANCH_ID = data.rows.branch_id;
                ...
                if(variable.isset(()=> data.rows.takeaway_address)) main.data_list.ADDRESS.TAKEAWAY = data.rows.takeaway_address;
                if(variable.isset(()=> data.rows.takeaway_address_names)) main.data_list.ADDRESS.TAKEAWAY_NAMES = data.rows.takeaway_address_names;
                if(variable.isset(()=> data.rows.branch_info)) main.data_list.BRANCH_INFO = data.rows.branch_info[0];
                if(variable.isset(()=> data.rows.branch_users)) main.data_list.BRANCH_USERS = data.rows.branch_users;
                if(variable.isset(()=> data.rows.work_times)) main.data_list.BRANCH_WORK_TIMES = data.rows.work_times;
            ...
            case main.set_type_for_data_list.INTEGRATIONS_RELATED_THINGS:
                if(variable.isset(()=> data.rows.users.rows)) main.data_list.INTEGRATION_USERS = data.rows.users.rows;
                if(variable.isset(()=> data.rows.types.rows)) main.data_list.INTEGRATION_TYPES = data.rows.types.rows;
                break;
...
function update_top_bar(){
    $(".e_company_title").html(array_list.find(main.data_list.BRANCHES,main.data_list.BRANCH_ID,"id").name)
}
$(function () {
    let _main = new main();
});
