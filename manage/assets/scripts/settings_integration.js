let settings_integration = (function () {
  ...
  let yemek_sepeti = {
    ...
    get: function () {
      let self = this;
      $(
        `${self.class_list.FORM}[type="${self.variable_list.SELECTED_TYPE}"]`
      ).autofill(
        array_list.find(
          main.data_list.INTEGRATION_USERS,
          self.variable_list.SELECTED_TYPE,
          "type"
        )
      );
    },
    get_products: function (data_clear = true) {
      ...
      function create_element() {
        ...
        switch (self.variable_list.SELECTED_TYPE) {
          case helper.db.integrate_types.YEMEK_SEPETI:
            data_integrate =
              self.variable_list.LIST_TYPE ===
              self.variable_list.list_types.PRODUCT
        ...
        (self.variable_list.LIST_TYPE === self.variable_list.list_types.PRODUCT
          ...
          if (
            self.variable_list.LIST_TYPE ===
            self.variable_list.list_types.OPTION
          ) {
            let option_name = array_list.find(
          ...
          let selected_product = array_list.find(
            self.variable_list.SELECTED_PRODUCTS,
            data.id,
            "product_id"
          );
        ...
        data_integrate.forEach((data) => {
          if (self.variable_list.search_values.INTEGRATE.length > 0) {
            if (
              !String(data.Name.toLocaleLowerCase("tr")).match(
                new RegExp(
                  self.variable_list.search_values.INTEGRATE.toLocaleLowerCase(
                    "tr"
                  ),
                  "gi"
                )
          ...
          if (typeof data_find !== "undefined") {
            ...
            let data_integrated_product =
              self.variable_list.LIST_TYPE ===
              self.variable_list.list_types.PRODUCT
                ? array_list.find(
                    main.data_list.PRODUCTS,
                    data_find.product_id,
                    "id"
                  )
                : array_list.find(
                    main.data_list.PRODUCT_OPTIONS_ITEMS,
            ...
            if (typeof data_integrated_product !== "undefined") {
              ...
              if (
                ...
                integrated_product.id = data_integrated_product.id;
            }
          }
          if (
            self.variable_list.LIST_TYPE ===
            ...
            let product_name = array_list.find(
              data_integrate_product,
              data.ProductId,
              "Id"
            );
      ...
      if (
        typeof self.variable_list.DATA[self.variable_list.SELECTED_TYPE] ===
          "undefined" ||
        self.variable_list.DATA[self.variable_list.SELECTED_TYPE].length < 1
      ) {
        self.variable_list.DATA[self.variable_list.SELECTED_TYPE] = {
          integrate: Array(),
          integrated: Array(),
        ...
        integrated_companies.yemek_sepeti.products.get(function (data) {
          self.variable_list.DATA[self.variable_list.SELECTED_TYPE].integrate =
            data.rows;
          get(
            get_types.PRODUCTS,
      ...
      $(`${self.id_list.MODAL_PRODUCTS} ${self.class_list.PRODUCTS}`).html(
        elements.products
      );
      $(
        `${self.id_list.MODAL_PRODUCTS} ${self.class_list.PRODUCTS_INTEGRATE}`
      ).html(elements.products_integrate);
    ...
    initialize: function () {
      ...
      function set_events() {
        ...
        $(self.id_list.FORM_ACCOUNT).submit(function (e) {
          ...
          if (object.status == 0) {
            set(set_types.ACCOUNT, form_data, function (data2) {
              console.log(data2);
              helper_sweet_alert.success(
                language.data.PROCESS_SUCCESS_TITLE,
                language.data.REGISTERED_ACCOUNT
              );
              main.get_integrate_related_things(
                      main.get_type_for_integrate_related_things.USERS
                    );
              page.fill_areas();
            });
          } else {
            integrated_companies.yemek_sepeti.restaurant_list.get(
              object,
              function (data) {
                if (
                  data.rows > 0 ||
                  typeof data.rows.RestaurantList !== "undefined"
                ) {
                  set(set_types.ACCOUNT, form_data, function (data2) {
                    console.log(data2);
                    helper_sweet_alert.success(
                  ...
                  helper_sweet_alert.error(
                    language.data.LOGIN_FAILED,
                    language.data.LOGIN_ERROR
                  );
                }
              }
            );
        ...
        $(self.class_list.PRODUCT_SHOW_BTN).on("click", function () {
          ...
          if (is_active) $(self.id_list.MODAL_PRODUCTS).modal("show");
            ...
            helper_sweet_alert.error(
              language.data.PROCESS_SUCCESS_TITLE,
              "Lütfen ilk önce hesap bölümünden 'Yemek Sepeti' hesabınıza giriş yapınız ve aktif ediniz."
            );
        });
        ...
        $(self.id_list.MODAL_PRODUCTS).on("show.bs.modal", function () {
          ...
          if (!$(self.class_list.PRODUCTS).hasClass("products-table-disabled"))
            $(self.class_list.TABLE_PRODUCTS).addClass(
        ...
        $(document).on(
          "click",
          `${self.class_list.PRODUCTS_INTEGRATE} tr`,
          function () {
            let element = $(this);

            let id = element.attr("product-id").toString();
            $(`${self.class_list.PRODUCTS_INTEGRATE} tr`).removeClass(
              "product-integrate-selected"
            );

            if (
              self.variable_list.SELECTED_PRODUCT_ID === id ||
              element.hasClass("product-integrate-selected-confirmed")
            ) {
              let id_integrated = $(
                `${self.class_list.PRODUCTS_INTEGRATE} tr[product-id="${id}"] [function="integrated_product"]`
              ).attr("product-id");
              $(
                `${self.class_list.PRODUCTS_INTEGRATE} tr[product-id="${id}"] [function="integrated_product"]`
              )
              ...
              $(
                `${self.class_list.PRODUCTS} tr[product-id="${id_integrated}"]`
              ).removeClass("product-selected-confirmed");
              ...
              $(self.class_list.TABLE_PRODUCTS).removeClass(
                "products-table-disabled"
              );
            }
        ...
        $(document).on("click", `${self.class_list.PRODUCTS} tr`, function () {
          ...
          $(
            `${self.class_list.PRODUCTS_INTEGRATE} tr[product-id="${self.variable_list.SELECTED_PRODUCT_ID}"] [function="integrated_product"]`
          )
            .html(`(${element.html()})`)
            .attr("product-id", id);
          $(
          ...
          if (
            ...
            element.addClass("product-selected-confirmed");
        });
        ...
        $(self.id_list.FORM_PRODUCTS).submit(function (e) {
          ...
          if (self.variable_list.SELECTED_PRODUCTS.length > 0) {
            set(
              ...
              {
                type: self.variable_list.SELECTED_TYPE,
                products: self.variable_list.SELECTED_PRODUCTS,
                list_type: self.variable_list.LIST_TYPE,
              },
              function (data) {
                data = JSON.parse(data);
                console.log(data);
                if (data.status) {
                  helper_sweet_alert.success(
                    language.data.PROCESS_SUCCESS_TITLE,
                    "Ürünler başarı ile eşleştirildi!"
                  );
                  self.variable_list.DATA[self.variable_list.SELECTED_TYPE] =
                    Array();
        ...
        $(`${self.id_list.FORM_ACCOUNT} select[name=status]`).on(
          "change",
          function () {
            let element = $(this);
            ...
            console.log(status);
            if (status == 0) {
              $(self.class_list.ACCOUNT_INFO).hide();
              $(self.class_list.ACCOUNT_INFO)
                .children("input")
                .removeAttr("required");
            } else {
              $(self.class_list.ACCOUNT_INFO).show();
              $(self.class_list.ACCOUNT_INFO)
                .children("input")
                .attr("required", true);
            }
