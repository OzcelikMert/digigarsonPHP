...
$(document).ready(function () {
  ...
  $("[open_page]").on("click", function () {
    pages.open($(this).attr("open_page"));
  });
  $("[close_page]").on("click", function () {
    pages.close($(this).attr("close_page"));
  });
  ...
  let company_details = {
    ...
    get: function (send = true, data) {
      ...
      if (send) {
        ...
        if (variable.isset(() => data.rows.BRANCH))
          main.data_list.BRANCH.INFO = data.rows.BRANCH[0];
        if (variable.isset(() => data.rows.PRODUCTS)) main.data_list.PRODUCTS = data.rows.PRODUCTS;
        if (variable.isset(() => data.rows.PRODUCT_LINKED_OPTIONS))
          main.data_list.PRODUCT_LINKED_OPTIONS = data.rows.PRODUCT_LINKED_OPTIONS;
        if (variable.isset(() => data.rows.OPTION_TYPES))
          main.data_list.OPTION_TYPES = data.rows.OPTION_TYPES;
        if (variable.isset(() => data.rows.PRODUCT_OPTIONS))
          main.data_list.PRODUCT_OPTIONS = data.rows.PRODUCT_OPTIONS;
        if (variable.isset(() => data.rows.PRODUCT_OPTION_ITEMS))
          main.data_list.PRODUCT_OPTION_ITEMS = data.rows.PRODUCT_OPTION_ITEMS;
        if (variable.isset(() => data.rows.CATEGORIES))
          main.data_list.CATEGORIES = array_list.sort(data.rows.CATEGORIES, "rank");
        if (variable.isset(() => data.rows.ACCEPTED_ADDRESS))
          main.data_list.ACCEPTED_ADDRESS = data.rows.ACCEPTED_ADDRESS;
        if (variable.isset(() => data.rows.WORK_TIMES))
          main.data_list.BRANCH.WORK_TIMES = data.rows.WORK_TIMES;
        if (variable.isset(() => data.rows.BRANCH_PAYMENT_TYPES))
          main.data_list.BRANCH.PAYMENT_TYPES = data.rows.BRANCH_PAYMENT_TYPES;
        if (variable.isset(() => data.rows.PAYMENT_TYPES))
          main.data_list.PAYMENT_TYPES = data.rows.PAYMENT_TYPES;
        if (variable.isset(() => data.rows.NOTIFICATIONS))
          main.data_list.NOTIFICATIONS = data.rows.NOTIFICATIONS;
        ...
        $(`${self.id_list.COMPANY_DETAILS} [function=title]`).html(
          main.data_list.BRANCH.INFO[self.branch_keys.NAME]
        );
        $(`${self.id_list.COMPANY_DETAILS} [function=address]`).html(
          main.data_list.BRANCH.INFO[self.branch_keys.ADDRESS]
        );
        $(`${self.id_list.COMPANY_DETAILS} [function=image]`).attr(
          "src",
          settings.paths.image.BRANCH_LOGO(branch_id)
        );
        $(`${self.id_list.COMPANY_DETAILS} [function=min_price]`).html(
          main.data_list.BRANCH.INFO[self.branch_keys.TAKE_AWAY_AMOUNT]
        );
        $(`${self.id_list.COMPANY_DETAILS} [function=min_time]`).html(
          main.data_list.BRANCH.INFO[self.branch_keys.TAKE_AWAY_AMOUNT]
        );
        ...
        switch (main.data_list.BRANCH.INFO.table_type) {
          ...
          case helper.db.branch_table_types.TABLE_WITHOUT_SESSION:
          ...
          case helper.db.branch_table_types.OTHER_SALE:
            ...
            break;
          case helper.db.branch_table_types.TAKE_AWAY:
            value = language.data.TAKEAWAY;
            break;
          case helper.db.branch_table_types.COME_TAKE:
            value = language.data.COME_TAKE;
            break;
          case helper.db.branch_table_types.DIGITAL_MENU:
            value = language.data.DIGITAL_MENU;
            $("#navbar_main").remove();
        ...
        if (
          ![
            helper.db.branch_table_types.TABLE,
            helper.db.branch_table_types.TABLE_WITHOUT_SESSION,
          ].includes(main.data_list.BRANCH.INFO.table_type)
        ) {
    ...
    get_product_details: function (change = false, basket_change = false) {
      ...
      if (!change) {
        //click on product to get option items but update price data when option selection changes
        let image = server.is_valid_url(product.image)
          ? product.image
          : settings.paths.image.PRODUCT(main.data_list.BRANCH.INFO.id) + "/" + product.image;
      ...
      self.variable_list.SELECTED_PRODUCT_TOTAL_PRICE =
        (self.variable_list.SELECTED_PRODUCT_OPTION_PRICE + product.price) *
        self.variable_list.SELECTED_PRODUCT_AMOUNT; // ((product + option) * qty) price
      $(`#product_detail_total_price`).html(
        self.variable_list.SELECTED_PRODUCT_TOTAL_PRICE.toFixed(2) + " ₺"
      );
    ...
    get_product_options: function (product) {
      ...
      linked_options.forEach((linked_option) => {
        ...
        options.forEach((option) => {
          let option_items = array_list.find_multi(
            main.data_list.PRODUCT_OPTION_ITEMS,
            option.id,
            "option_id"
          );
      ...
      function create_element_items(type, option_item, is_default = 0) {
        ...
        let show_price =
          option_item.price !== 0
            ? `` + (option_item.price > 0 ? `+` : "") + `${option_item.price.toFixed(2)}`
            : "";
        switch (type) {
          case self.option_types.MATERIALS:
            ...
            for (const i2 in opt.items) {
              ...
              if (item !== undefined) {
                if (opt.type === self.option_types.MATERIALS) {
                  materials += `${
                    array_list.find(main.data_list.PRODUCT_OPTION_ITEMS, item.id, "id").name
                  } `;
                } else {
                  options += `<li>${
                    array_list.find(main.data_list.PRODUCT_OPTION_ITEMS, item.id, "id").name
				...
				<div class="order e_order animate__animated animate__fadeInUp animate__faster" index="${index}">
					<div class="product">
						<div class="image">
							...
							<img src="${
                server.is_valid_url(data.product.image)
                  ? data.product.image
                  : `../images/branches/${branch_id}/product/${data.product.image}`
              }" alt="img">
						</div>
						<div class="content">
							<div class="title"><span>${data.product.name}</span></div>
							<div class="price e_price"><span class="price">${parseFloat(data.product.total_price).toFixed(
                2
              )}₺</span></div>
						</div>
						<div class="amount">
        ...
        success: function (data) {
          ...
          if (form_data.set_type === self.set_type.GET_BRANCH) {
            ...
            if (data.status) {
              ...
              $("page").html(
                `<div class="text-center" style="padding-top:calc(50vh - 40px)"><h3>${language.data.NOT_RECOGNIZED_QR}</h3><p>${language.data.INVALID_QR_CODE}</p></div>`
              );
            }
    ...
    set_order: function (order, takeaway = null, come_take = null) {
      ...
      $.ajax({
        ...
        data: {
          ...
          takeaway: takeaway,
          come_take: come_take,
          set_type: self.set_type.SET_ORDER,
        },
        success: function (data) {
          console.log(data);
          data = JSON.parse(data);
    ...
    selected_option_values: function () {
      ...
      Array.from(
        $(
          "#page_product_details [area=options] [option-id],#page_product_details [area=options] [option-id]"
        )
      ).forEach(function (e) {
        let element = $(e);
    ...
    create_elements(element_type, data) {
      ...
      function create_category_elements(data) {
        let image =
          data.product_id > 0
            ? array_list.find(main.data_list.PRODUCTS, data.product_id, "id")
            : "";
        image = typeof image === "undefined" ? "" : image.image;

        return `<div class="box" category-id="${data.id}">
					<div class="image"><img src="${
            server.is_valid_url(image)
              ? image
              : settings.paths.image.BRANCH_LOGO(main.data_list.BRANCH.INFO.id)
          }"  alt="${data.name}">
					</div>
      ...
      function create_product_elements(data) {
        let image = server.is_valid_url(data.image)
          ? data.image
          : settings.paths.image.PRODUCT(main.data_list.BRANCH.INFO.id) + "/" + data.image;
        return `
    ...
    initialize: function () {
      ...
      function set_events() {
        $(document).on("_pop_pages", function (event) {
          switch (event.detail.type) {
            case pages.event_types.CLOSE:
              if (
                event.detail.name === "product_details" ||
                event.detail.name === "login_and_register"
              ) {
                $("#navbar_main").show();
              ...
              break;
            case pages.event_types.OPEN:
            case pages.event_types.CLOSE_ALL:
        ...
        $(self.class_list.SELECT_LANGUAGE).click(function () {
          ...
          languages.forEach(
            (item) =>
              (element += `<button language="${item.language}" class="btn btn-primary w-100 mb-2">${item.name}</button>`)
          );
          Swal.fire({
            showCancelButton: false,
            showConfirmButton: false,
        ...
        $(document).on(
          "click",
          `${self.id_list.COMPANY_DETAILS} .categories .list .box`,
          function () {
            let element = $(this);
        ...
        $(document).on(
          "click",
          `${self.id_list.PRODUCT_DETAILS} .e_product_operation`,
          function () {
            let element = $(this);
        ...
        $(document).on("click", `${self.id_list.BASKET} .e_product_operation`, function () {
          ...
          let count_element = $(closest)
            .children(".product")
            .children(".amount")
            .children(".e_product_count");
          let type = element.attr("function");
          let count = parseInt(count_element.html());
          let one_price =
            self.variable_list.BASKET[index].product.total_price /
            self.variable_list.BASKET[index].product.amount;
          switch (type) {
            case "+":
              if (count < 30) {
                count++;
          ...
          if (type === "-" || type === "+") {
            ...
            $(`#page_basket .e_order[index="${index}"] .e_price span`).html(
              self.variable_list.BASKET[index].product.total_price + "₺"
            );
          }
          self.variable_list.SELECTED_PRODUCT_AMOUNT = count;
        ...
        $(document).on("click", `${self.id_list.BASKET} [function=send]`, function () {
          switch (main.data_list.BRANCH.INFO.table_type) {
            ...
            case helper.db.branch_table_types.TAKE_AWAY:
              let total_price = 0;
              self.variable_list.BASKET.forEach(function (item) {
                total_price += item.product.total_price;
              });
        ...
        $(document).on("change", `${self.id_list.ORDER_CONFIRM} [area]`, function () {
          ...
          switch (area) {
            case "payment-selection":
              if (element.val() > 0) {
                let payment = $(
                  `${self.id_list.ORDER_CONFIRM} [area=payment-selection] option:selected`
                ).text();
            ...
            case "address-selection":
              ...
              if (status) {
                $(`${self.id_list.ORDER_CONFIRM} [area=address-alert]`)
                  .html(`${language.data.VALID_ADDRESS}`)
                  .removeClass("alert-info alert-danger")
                  .addClass("alert-success")
                  .show();
                self.variable_list.SEND_TAKEAWAY.ADDRESS_SELECT = true;
                self.variable_list.SEND_TAKEAWAY.USER_ADDRESS_ID = $(
                  `${self.id_list.ORDER_CONFIRM} [area=address-selection] option:selected`
                ).attr("user-select-id");
              } else {
                self.variable_list.SEND_TAKEAWAY.ADDRESS_SELECT = false;
                self.variable_list.SEND_TAKEAWAY.USER_ADDRESS_ID = 0;
          ...
          if (
            self.variable_list.SEND_TAKEAWAY.PAYMENT_SELECT &&
            self.variable_list.SEND_TAKEAWAY.ADDRESS_SELECT
          ) {
            $(`${self.id_list.ORDER_CONFIRM} button[function=send-takeaway]`).prop("disabled", "");
          } else {
            $(`${self.id_list.ORDER_CONFIRM} button[function=send-takeaway]`).prop(
              "disabled",
              "disabled"
            );
          }
        ...
        $(document).on(
          "click",
          `${self.id_list.ORDER_CONFIRM} [function=send-takeaway]`,
          function () {
            Swal.fire({
...
let page_address = (function () {
  ...
  let address = {
    ...
    add_address: function () {
      let self = this;
      $(
        "#page_edit_address form input,#page_edit_address form select,#page_edit_address form textarea"
      )
        .val("")
        .html("");
    ...
    get_address_select: function (data, type) {
      ...
      if (type === undefined) {
        type = self.variable_list.SELECT_TYPE;
      }
      switch (type) {
        case self.type.CITY:
          $("#page_edit_address [function=1]").html(
            self.create_options(main.data_list.ADDRESS.CITY, "id", "city")
          );
          break;
        case self.type.TOWN:
        ...
        case self.type.NEIGHBORHOOD:
          $("#page_edit_address [function=4]").html(
            self.create_options(data, "id", "neighborhood")
          );
          break;
    ...
    initialize: function () {
      ...
      function set_events() {
        $(document).on("change", "#page_edit_address .address_list select[function]", function () {
          ...
          switch (type) {
            case self.type.CITY:
              ...
              $("#page_edit_address [function=3],#page_edit_address [function=4]").html(
                self.create_options()
              );
              break;
            case self.type.TOWN:
          ...
          if (data["next_type"] > 0) {
            ...
            self.set(data, function (data) {
              self.get_address_select(data);
            });
          }
        });
        // send form data //
        ...
        $(document).on(
          "click",
          "#page_address_list [function=address_box] [operation]",
          function () {
            let element = $(this);
